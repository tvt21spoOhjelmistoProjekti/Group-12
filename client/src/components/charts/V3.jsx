...
const V3_V4 = ({ V3_V4_data, V10_Data }) => {
    ...
    const [dataLink_Monthly, setDataLink_Monthly] = useState([])                     // Declaring variables
    ...
    const getData = async () => {
        try {
            ...
            if (!V3_V4_data && !V10_Data) {
                ...
                response = await axios.get(process.env.REACT_APP_REQUEST_URL + "chart/V3_V4", config)   //Chart data from database
            ...
            setSourceLinkUrl(response.data[0].sourceLinkUrl)                   //Database links and description to variables
                ...
                datasets: [
                    {
                        ...
                        data: response.data.filter(d => d.Mean_Annual != 0).map(d => ({ time: new Date(d.Time_Annual + "-01-01"), value: d.Mean_Annual })), //Mapping through database to setup dataset for chart
                        borderColor: "black",
                    ...
                    },
                    {
                        ...
                        data: response.data.map(d => ({ time: new Date(d.Time_Montly + "-" + d.Month), value: d.Average_Monthly })), //Mapping through database to setup dataset for chart
                        borderColor: "#000AFF",
                        borderWidth: 1,
                    ...
                    {
                        label: "Optional DSS",
                        data: response.data.filter(d => d.DSS_Year_AD != 0).map(d => ({ time: new Date(d.DSS_Year_AD + "-01-01"), value: d.DSS_CO2_Mixing_Ratio })), //Mapping through database to setup dataset for chart
                        borderColor: "green",
                        backgroundColor: "grey",
                        borderWidth: 2,
                    ...
                    {
                        label: "Optional DE",
                        data: response.data.filter(d => d.DE_Year_AD != 0).map(d => ({ time: new Date(d.DE_Year_AD + "-01-01"), value: d.DE_CO2_Mixing_Ratio })), //Mapping through database to setup dataset for chart
                        borderColor: "purple",
                        backgroundColor: "grey",
                        borderWidth: 2,
                    ...
                    {
                        label: "Optional DE08",
                        data: response.data.filter(d => d.DE08_Year_AD != 0).map(d => ({ time: new Date(d.DE08_Year_AD + "-01-01"), value: d.DE08_CO2_Mixing_ratio })), //Mapping through database to setup dataset for chart
                        borderColor: "red",
                        backgroundColor: "grey",
                        borderWidth: 2,
                    ...
                    },
                    {
                        ...
                        data: response2.data.filter(d => d.Year > 1100).map(d => ({ time: new Date(d.Year + "-01-01"), value: d.value, event: d.Event })), //Mapping through database to setup dataset for V10 chart
                        borderColor: "black",
